 ...
 *
 * Regel:
 *   aktuell in State F44
 *   UND  (heute - D_ALSLN_RT_NXT_DUE_DATE) >= 4
 *   UND  N_ALSLN_DQ_TOT_AMT_PDUE > 0
 *   UND  keine Rücklastschrift (TRAN_CD='9583', GEN_IND='N') im
 *        Transaktions-Feed seit D_ALSLN_RT_NXT_DUE_DATE
 *
 * Quellen:
 *   vedw.Dri_276_ed_als_loan_nc   Kreditstamm (State, Fälligkeit, Rückstand)
 *   vedw.Dri_276_ed_als_txn_nc    Transaktionen (RL = 9583 / N)
 *
 * HINWEIS: V_ALSLN_PRI_FUNC_STATE und V_ALSLN_ORIG_ACCT_NBR sind die
 *          vermuteten Spaltennamen im loan_nc (analog _ALSTXN_). Falls im
 *          DD anders benannt, bitte in den 3 Zeilen unten anpassen.
 *****************************************************************************/
...
proc sql;
    ...
    select  l.V_ALSLN_ORIG_ACCT_NBR                    as A_ACC,
            l.V_ALSLN_PRI_FUNC_STATE                   as C_STATE,
    ...
    from    vedw.Dri_276_ed_als_loan_nc l
    where   l.MIS_DATE               = &today.
      and   l.V_ALSLN_PRI_FUNC_STATE = 'F44'
      and   (&today. - l.D_ALSLN_RT_NXT_DUE_DATE) >= 4
      and   l.N_ALSLN_DQ_TOT_AMT_PDUE > 0
      and   not exists (
                select 1
                from   vedw.Dri_276_ed_als_txn_nc t
                where  t.V_ALSTXN_ORIG_ACCT_NBR = l.V_ALSLN_ORIG_ACCT_NBR
